...
public class TurnDeviceOnActionProvider extends ActionProvider {
    private final ActionsPlugin plugin;

    public TurnDeviceOnActionProvider(ActionsPlugin plugin) {
        super(
            PropertyContainerClassContext.create(plugin.getContext(), "turnOn"),
            "Turn on bulbs or switches",
            "Turn on {devices}",
            true,
            1000
        );
    ...
    public SingleAction createAction(final Map<String,Object> properties) {
        return new TurnDeviceOnAction(plugin.getContext(), new DeviceControlExecutionContext(plugin, properties),plugin.getEventLoopExecutor());
    }

    private class TurnDeviceOnAction extends AbstractVariableUpdateAction {
        TurnDeviceOnAction(PluginContext pluginContext, ActionExecutionContext executionContext, EventLoopExecutor executor) {
